...
var nearConfig = getConfig(process.env.NODE_ENV || "development");
}
...
async function initContract() {
  ...
  if (typeof window !== undefined) {
  var near = await nearAPI.connect(Object.assign({ deps: { keyStore: new nearAPI.keyStores.BrowserLocalStorageKeyStore() } }, nearConfig));
  }
  // Initializing Wallet based Account. It can work with NEAR TestNet wallet that
  // is hosted at https://wallet.testnet.near.org

  if (typeof window !== undefined) {
  var walletAccount = new nearAPI.WalletAccount(near);
  }
  // Getting the Account ID. If unauthorized yet, it's just empty string.

  if (typeof window !== undefined) {
    var accountId = window.walletAccount.getAccountId();
  }

  // Initializing our contract APIs by contract name and configuration.

var contract = await window.near.loadContract(nearConfig.contractName, {
...
var nearInitPromise = initContract()
  .then(doWork)
  .catch(console.error);
